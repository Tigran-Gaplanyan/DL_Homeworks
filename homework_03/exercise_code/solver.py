...
class Solver(object):
    ...
    def _step(self):
        ...
        model_forward, _ = model(X_train)
        dLoss_dhaty = loss_func.backward(model_forward, y_train)
        dhaty_dW = model.backward(model_forward)
        dLoss_dW = np.dot(dhaty_dW.T, dLoss_dhaty) / X_train.shape[0]
        opt.step(dLoss_dW)
