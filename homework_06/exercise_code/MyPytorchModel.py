...
class MyPytorchModel(pl.LightningModule):
    ...
    def general_step(self, batch, batch_idx, mode):
        ...
        # loss
        loss = F.cross_entropy(out, targets).to(torch.float32)
    ...
    def general_end(self, outputs, mode):
        ...
        acc = torch.tensor(acc.astype(np.float32), dtype=torch.float32)
        return avg_loss, acc
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append({'val_loss': loss, 'val_n_correct':n_correct, 'val_n_total': n_total})
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('val_acc', acc, logger=True, prog_bar=True)
...
class CIFAR10DataModule(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        arg_dict = self.return_dataloader_dict('test')
        return DataLoader(self.dataset["test"], **arg_dict)
