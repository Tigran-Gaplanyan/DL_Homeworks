...
class CSVDataset(Dataset):
    ...
    def __init__(self, target_column, transform=None, mode="train", input_data=None, *args, **kwargs):
        ...
        if input_data is not None:
            ...
            dataset_csv_name = 'housing_train.csv'
...
def get_homework_04_transform():
    # dataloading and preprocessing steps as in homework_03
    target_column = 'SalePrice'
    dl_homeworks_path = os.path.dirname(os.path.abspath(os.getcwd()))
    root_path = os.path.join(dl_homeworks_path, "datasets", 'housing')
    housing_file_path = os.path.join(root_path, "housing_train.csv")
    download_url = 'https://drive.google.com/uc?export=download&id=1e4n-oFdqpGgWxHVx1IuGvIggdaUqRqpY'

    # Always make sure this line was run at least once before trying to
    # access the data manually, as the data is downloaded in the
    # constructor of CSVDataset.
    train_dataset = CSVDataset(target_column=target_column, root=root_path, download_url=download_url, mode="train", dataset_zip_name = "housing_train.csv")
    ...
    mn, mx, mean = df[selected_columns].min(), df[selected_columns].max(), df[selected_columns].mean()
